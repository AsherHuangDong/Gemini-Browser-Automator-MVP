...
description: 使用 Playwright 自动化与 Google Gemini 聊天，支持持久化登录、流式输出、文件上传、自动代理检测和默认 headless 模式
version: 1.1.0
type: skill
tools:
  bash:
...
3. **加载登录态**：加载保存的登录态和 Cookies
4. **严格登录检查**：使用多重条件判断登录状态（URL + 输入框 + 账号元素）
5. **智能 fallback**：
   - 如果 headless 模式下登录失败 → 自动切换到 headful 模式
   - 弹出浏览器窗口，提示用户手动登录
   - 登录成功后保存登录态，下次继续使用 headless 模式
6. **健康检查**：每次发送消息前检查 session 是否有效
7. **消息发送**：将用户输入的文本发送到 Gemini
8. **流式输出**：使用 MutationObserver 实时监听 DOM 变化，逐字输出回复
9. **文件处理**：支持上传文件，使用 Playwright 的 filechooser 事件拦截
10. **异常恢复**：自动处理超时、网络错误等异常情况
